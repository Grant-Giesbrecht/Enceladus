function show_single_stub(Z_L, Z_S, f0, freqs, Zline, Zstub)
% SHOW_SINGLE_STUB Shows single stub match solutions
%
%	SHOW_SINGLE_STUB(ZL, ZS, F0) Show single stub solutions matching from ZL
%	to ZS, at a center frequency of F0. 50 ohm lines are used.
%
%	SHOW_SINGLE_STUB(ZL, ZS, F0, FREQS) Plot results over frequencies
%	specified in FREQS.
%
%	SHOW_SINGLE_STUB(ZL, ZS, F0, FREQS, ZLINE, ZSTUB) Specify the
...
% See also show_double_stub.
	...
	% Find solutions
	solns = Lmatch(Z_S, Z_L, Zline, Zstub, freqs, f0);
	...
	mt.title("Bandwidth Summary");
	displ(mt.str());
	
	figure(1);
	plotBandwidth(solns, freqs, 2, 2, -10);
