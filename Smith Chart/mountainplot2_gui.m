...
% 	title("PAE at 10 GHz, iPower = idx 7");
	...
	hold off;
	contoursc(lp_filt.gamma(), lp_filt.getArrayFromName(param1), 'ContourLabel', p.Results.ContourLabel, 'Scheme', p.Results.Scheme);
	s1 = surfsc(lp_filt.gamma(), lp_filt.getArrayFromName(param1), 'ZLabel', p.Results.ContourLabel);
	alpha(s1, alpha1);
	%set(s1, 'EdgeColor', 'interp', 'FaceColor', 'interp');
	s2 = surfsc(lp_filt.gamma(), lp_filt.getArrayFromName(param2).*30);
	alpha(s2, alpha2);
	%set(s2, 'EdgeColor', 'interp', 'FaceColor', 'interp');
	title(param1 + " at " + freq./1e9 + " GHz and iPower = " + iPwr);
