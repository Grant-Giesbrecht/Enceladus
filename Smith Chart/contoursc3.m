function [h, contours_out] = contoursc3(gamma, val, varargin)
...
% 	contour_color = [.7667, .5157, .9765];
	...
	for arr = sa
		...
		% Get height array
		heights = repmat(arr.level*scale_fact, 1, numel(g));
		
		h = plotsc3(g, heights, 'Color', contour_color, 'Marker', 'None', 'LineStyle', '-', 'ContourValue', arr.level, 'ContourLabel', p.Results.ContourLabel, 'Scheme', p.Results.Scheme);
