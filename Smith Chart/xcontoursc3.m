function [h, contours_out] = xcontoursc3(lp, val_name, varargin)
...
%
	...
	p.addParameter('HeightMax', [], @isnumeric);
	p.parse(varargin{:});
...
% 		for arr = sa
	...
	scale_fact = 1;
	if ~isempty(p.Results.HeightMax)
		scale_fact = p.Results.HeightMax/max(val);
	end	
	...
	for c = master_contours
		...
		% Get height array
		heights = repmat(master_levels(count)*scale_fact, 1, numel(g));
...
% 		h = plot(c, 'EdgeColor', 'none');
		try
			ph = plotsc3(g, heights, 'Color', contour_color, 'Marker', 'None', 'LineStyle', '-', 'ContourValue', arr.level, 'ContourLabel', p.Results.ContourLabel, 'Scheme', p.Results.Scheme);
		catch
			displ("Oops!");
		end
